...
# Gespielt zählen und Ja zu 1 und Nein zu 0 umwandeln
df["Gespielt"] = df["Gespielt"].apply(lambda x: 1 if x == "Ja" else 0)


# Plus-Minus und Punkte berechnen
df["PlusMinus"] = (df["Plus"] - df["Minus"]) / sum(df["Gespielt"])
df["Punkte"] = df["T"] + df["A"]
